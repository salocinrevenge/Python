...
class Mapa:
    ...
    def colisionRectMap(self, x1, y1, tamanho, id):
        if self.mapa[y1 // ESCALA][x1 // ESCALA] is "#" or (
                self.mapa[y1 // ESCALA][x1 // ESCALA] is "@" and id is "Pac") or \
                self.mapa[(y1 + tamanho - 1) // ESCALA][x1 // ESCALA] is "#" or (
                self.mapa[(y1 + tamanho - 1) // ESCALA][
                    x1 // ESCALA] is "@" and id is "Pac") or self.mapa[y1 // ESCALA][
            (x1 + tamanho - 1) // ESCALA] is "#" or \
                (self.mapa[y1 // ESCALA][(x1 + tamanho - 1) // ESCALA] is "@" and id is "Pac") or \
                self.mapa[(y1 + tamanho - 1) // ESCALA][
                    (x1 + tamanho - 1) // ESCALA] is "#" or (self.mapa[(y1 + tamanho - 1) // ESCALA][
                                                                 (
                                                                         x1 + tamanho - 1) // ESCALA] is "@" and id is "Pac") or (
                self.mapa[y1 // ESCALA][x1 // ESCALA] is "/" and id is "Ghost") or (
                self.mapa[(y1 + tamanho - 1) // ESCALA][x1 // ESCALA] is "/" and id is "Ghost") or (
                self.mapa[y1 // ESCALA][(x1 + tamanho - 1) // ESCALA] is "/" and id is "Ghost") or (
                self.mapa[(y1 + tamanho - 1) // ESCALA][(x1 + tamanho - 1) // ESCALA] is "/" and id is "Ghost"):
            return True
        else:
            return False
    ...
    def colisionRectFood(self, x1, y1, tamanho):
        global score_value
        if self.mapa[y1 // ESCALA][x1 // ESCALA] is ".":
            self.mapa[y1 // ESCALA][x1 // ESCALA] = " "
            score_value += 1
            if sound:
                food_sound.play()
        elif self.mapa[(y1 + tamanho - 1) // ESCALA][x1 // ESCALA] is ".":
            self.mapa[(y1 + tamanho - 1) // ESCALA][x1 // ESCALA] = " "
            score_value += 1
            if sound:
                food_sound.play()
        elif self.mapa[y1 // ESCALA][(x1 + tamanho - 1) // ESCALA] is ".":
            self.mapa[y1 // ESCALA][(x1 + tamanho - 1) // ESCALA] = " "
            score_value += 1
            if sound:
                food_sound.play()
        elif self.mapa[(y1 + tamanho - 1) // ESCALA][(x1 + tamanho - 1) // ESCALA] is ".":
...
class Pacman:
    ...
    def tick(self, mapa):
        ...
        if not newWalk and self.validos[0] and self.request is "leste":
            self.velY=0
            self.velX= self.acelleration
        elif not newWalk and self.validos[1] and self.request is "norte":
            self.velY= -self.acelleration
            self.velX= 0
        elif not newWalk and self.validos[2] and self.request is "oeste":
            self.velY=0
            self.velX= -self.acelleration
        elif not newWalk and self.validos[3] and self.request is "sul":
            self.velY= self.acelleration
            self.velX= 0
...
class BlueGhost:
    ...
    def inteligencia(self, mapa):
        if self.mapa[self.y // ESCALA][self.x // ESCALA] is "B":
            self.velY = -self.acceleration
        elif self.mapa[self.y // ESCALA][self.x // ESCALA] is "." or self.mapa[self.y // ESCALA][
            self.x // ESCALA] is "P" or self.mapa[self.y // ESCALA][self.x // ESCALA] is " ":
            if (self.nvalidos > 2 or mapa.colisionRectMap(self.x + self.velX, self.y + self.velY, ESCALA,
                ...
                while True:
                    ...
                    if rand is 0 and self.validos[0]:
                        self.velX = self.acceleration
                        break
                    elif rand is 1 and self.validos[1]:
                        self.velY = -self.acceleration
                        break
                    elif rand is 2 and self.validos[2]:
                        self.velX = -self.acceleration
                        break
                    elif rand is 3 and self.validos[3] and not mapa.colisionRectMap(self.x, self.y + ESCALA, ESCALA,
                                                                                    "Pac"):
                        self.velY = self.acceleration
                        break
...
class RedGhost:
    ...
    def write(self):
        for i in range(self.nlinhas):
            for j in range(self.ncolunas):
                if self.mapa2[i][j] is not "\n":
                    print(self.mapa2[i][j] + " ", end="")

    def limparCaminhos(self):
        for i in range(self.nlinhas):
            for j in range(self.ncolunas):
                if self.mapa2[i][j] is "<" or self.mapa2[i][j] is ">" or self.mapa2[i][j] is "A" or self.mapa2[i][j] is "V" or self.mapa2[i][j] is "R" or self.mapa2[i][j] is "P" or self.mapa2[i][j] is "B":
                    self.mapa2[i][j] = "."
        ...
        while processando:
            ...
            for i in range(nnovos):
                if novos[0][0] ==(self.y//ESCALA) and novos[0][1] == (self.x//ESCALA):
                    processando = False
                    break
                if self.mapa2[novos[0][0]+1][novos[0][1]] is "." or self.mapa2[novos[0][0]+1][novos[0][1]] is " " or self.mapa2[novos[0][0]+1][novos[0][1]] is "@" or self.mapa2[novos[0][0]+1][novos[0][1]] is "R":
                    self.mapa2[novos[0][0]+1][novos[0][1]] = "A"
                    tmp += 1
                    novos.append((novos[0][0]+1, novos[0][1]))
                if self.mapa2[novos[0][0]][novos[0][1]-1] is "." or self.mapa2[novos[0][0]][novos[0][1]-1] is " " or self.mapa2[novos[0][0]][novos[0][1]-1] is "@" or self.mapa2[novos[0][0]][novos[0][1]-1] is "R":
    ...
    def seguirCaminho(self):
        if self.mapa2[self.y//ESCALA][self.x//ESCALA] is "<" and naGrade(self.x, self.y,ESCALA-self.acceleration):
            self.velX = -self.acceleration
            self.velY = 0
        elif self.mapa2[self.y//ESCALA][self.x//ESCALA] is ">" and naGrade(self.x, self.y,ESCALA-self.acceleration):
            self.velX = +self.acceleration
            self.velY = 0
        elif self.mapa2[self.y//ESCALA][self.x//ESCALA] is "A" and naGrade(self.x, self.y,ESCALA-self.acceleration):
            self.velY = -self.acceleration
            self.velX = 0
        elif self.mapa2[self.y//ESCALA][self.x//ESCALA] is "V" and naGrade(self.x, self.y,ESCALA-self.acceleration):
            self.velY = +self.acceleration
            self.velX = 0
        else:
            pass
...
class OrangeGhost:
    ...
    def seguirCaminho(self):
        if self.mapa2[self.y//ESCALA][self.x//ESCALA] is "P" and naGrade(self.x, self.y,ESCALA-self.acceleration):
            self.velY = 0
            self.velX = 0
        elif self.mapa2[self.y//ESCALA][self.x//ESCALA] is "<" and naGrade(self.x, self.y,ESCALA-self.acceleration):
            self.velX = -self.acceleration
...
class GreenGhost:
    ...
    def inteligencia(self, mapa2):
        if self.velX is 0 and self.velY is 0:
            for i in range(self.nlinhas):
                for j in range(self.ncolunas):
                    if mapa2.mapa[i][j] is ".":
...
def inicializar():
    global lives, score_value, mapachar,mapa1,player,blueGhost,redGhost,orangeGhost,greenGhost,pause
    f = open("mapa.txt", "r")
    mapachar = []
    # carregar mapachar
    for i in range(altura):
...
while running:
    ...
    while playing:
        ...
        if (colideRect(player, ESCALA, blueGhost, ESCALA) or colideRect(player, ESCALA, redGhost, ESCALA) or colideRect(player, ESCALA, orangeGhost, ESCALA) or colideRect(player, ESCALA, greenGhost, ESCALA)) and player.ivuneravelTime is 0 and not debug:
            lives -= 1
